...
*/
  ...
  #pragma acc kernels loop  //Applying kernel check and parallel loop
  #pragma acc data
...
#pragma acc kernels //Applying kernel check for parallelization of loops
void compute ( int np, int nd, double pos[], double vel[], 
  double mass, double f[], double *pot, double *kin )
...
*/
    ...
    #pragma acc loop vector //applying parallel loop vector
    for ( j = 0; j < np; j++ )
    {
      if ( k != j )
      {
...
*/
{
  ...
  d = 0.0;
  #pragma acc loop // Parallelizes the loop following this directive across the accelerator device.
  for ( i = 0; i < nd; i++ )
  {
    dr[i] = r1[i] - r2[i];
    d = d + dr[i] * dr[i];
...
*/
  #pragma acc parallel loop //parallelize the loop
  for ( i = 0; i < nd; i++ )
  {
    for ( j = 0; j < np; j++ )
    {
...
# pragma omp for
  #pragma acc parallel loop //for parallelize the looping
  for ( j = 0; j < np; j++ )
  {
    for ( i = 0; i < nd; i++ )
    {
